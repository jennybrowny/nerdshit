...
void SaveState::createSlotVisuals() {
    for (size_t i = 0; i < saveSlots.size(); i++) {
        ...
        // ENHANCED: Individual Clear button for each slot (only if slot exists)
        if (slot.exists) {
...
void SaveState::handleEvents(Game& game) {
    ...
    while (auto event = game.getWindow().pollEvent()) {
        if (event->is<sf::Event::Closed>()) {
            ...
            if (deleteButton->isClicked(mousePos) && selectedSlot >= 0) {
                deleteSelectedSave();
                return;
            }
            ...
            // ENHANCED: Check individual Clear buttons for each slot
            for (size_t i = 0; i < saveSlots.size(); i++) {
...
void SaveState::update(Game& game) {
    ...
    deleteButton->setVisible(selectedSlot >= 0 && selectedSlot < static_cast<int>(saveSlots.size()) && saveSlots[selectedSlot].exists);
...
void SaveState::render(Game& game) {
    ...
    if (deleteButton->isVisible()) deleteButton->draw(game.getWindow());
...
void SaveState::deleteSelectedSave() {
    if (selectedSlot >= 0 && selectedSlot < static_cast<int>(saveSlots.size()) && saveSlots[selectedSlot].exists) {
        std::string saveFile = "savegame_slot" + std::to_string(saveSlots[selectedSlot].slotNumber) + ".dat";
        
        if (std::filesystem::remove(saveFile)) {
            std::cout << "Save slot " << (selectedSlot + 1) << " deleted successfully." << std::endl;
...
// NEW: Clear all saves functionality
...
// NEW: Clear specific save functionality
